...
- `monte_carlo_rollout_query_weighting_mode`
  - Controls whether rollout queries are reweighted before Monte Carlo training.
  - `phase_balanced`: current behavior. Full-block, feasible-tail, and frontier-tail queries are balanced by their configured phase weights, so dense `n_kl` regions do not dominate just because they produced more samples.
  - `uniform_per_query`: disables this balancing. Every visited rollout query keeps weight `1.0`, so there are no extra `n_kl` rollout weights.
...
- `monte_carlo_rollout_query_weighting_mode`
  - Controls whether downlink joint rollout queries are phase-balanced before training.
  - `phase_balanced`: uses the configured full-block / feasible-tail / frontier-tail weights and normalizes by how many queries were generated in each phase.
  - `uniform_per_query`: disables this balancing and trains with one equal weight per visited rollout query.
