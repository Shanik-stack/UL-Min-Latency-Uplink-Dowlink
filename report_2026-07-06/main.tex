...
notation layer, defines the communication scenarios, derives the
...
\begin{enumerate}
    ...
    \item define the communication scenarios that determine what each block is asked to deliver,
...
$b_{k,\ell}^{\mathrm{req}}$ & Requested bits to send for user $k$ in block $\ell$. In payload completion, $b_{k,\ell}^{\mathrm{req}} = B_{k,\mathrm{rem}}^{(\ell)}$; in fixed continuous communication, $b_{k,\ell}^{\mathrm{req}} = \bar b_{k,\ell}$. \\
$b_{k,\ell}$ & Bits actually sent to user $k$ in block $\ell$. \\
$B_{k,\mathrm{rem}}^{(\ell)}$ & Remaining unsent bits of user $k$ when block $\ell$ starts in payload-completion mode. \\
$\bar b_{k,\ell}$ & Target bits to send for user $k$ in block $\ell$ in fixed continuous communication mode. \\
...
Before separating uplink and downlink, it is useful to define the two traffic
interpretations used throughout the report. These scenarios determine what each
block is asked to transmit, how latency is accumulated across blocks, and how
unserved bits are interpreted.
...
\end{equation}
Thus, payload completion is a workload-draining problem: every decision is made
with respect to the residual bits that still have to be delivered.

\subsection{Continuous Communication (Fixed Bits per Block)}

In continuous communication mode, the transmission length $L$ (total no. of sub-blocks) is known in
advance and the blockwise targets are specified externally:
\begin{equation}
b_{k,\ell}^{\mathrm{req}}=\bar b_{k,\ell},
\qquad \ell=1,\dots,L.
...
\end{equation}
which is recorded as an incomplete transmission of bits rather than being carried over to next sub-block. Thus, fixed continuous communication is a
prescribed-demand problem: each block arrives with an externally given target.
...
For uplink fixed continuous communication, the the block count
$L_k$ is set in
...
For downlink fixed continuous communication, the the block count
$L_k$ is prescribed and
...
Suppose block $\ell$ of user $k$ is asked to serve
$b_{k,\ell}^{\mathrm{req}}$ bits. In payload completion,
$b_{k,\ell}^{\mathrm{req}} = B_{k,\mathrm{rem}}^{(\ell)}$; in fixed continuous
communication, $b_{k,\ell}^{\mathrm{req}} = \bar b_{k,\ell}$. The objective is
the total latency of user $k$, namely
\(
\tau_k = \frac{1}{f_{s,k}}\sum_{\ell'} n_{k,\ell'}
\).
When block $\ell$ is being designed, all terms except the current block are
already fixed or deferred to the remaining schedule, so the blockwise design is
written in the total-latency form
\begin{subequations}
\begin{align}
...
\text{s.t.}\quad
&
\frac{b_{k,\ell}^{\mathrm{req}}}{n_{k,\ell}}
\le
R_{k,\ell}^{(\UL)},
...
In fixed continuous communication mode, the the block count $L_k$ is prescribed in
advance, and each block carries a target $\bar b_{k,\ell}$. Here
$b_{k,\ell}$ denotes the transmitted bits of the block, whereas the design
variables are the sub-blocklength and precoder. The problem becomes
\begin{subequations}
\begin{align}
\min_{\{n_{k,\ell},\F_{k,\ell}^{(\UL)}\}}
\quad
...
\\
&
b_{k,\ell} \in \{1,\dots,\bar b_{k,\ell}\},
\qquad \forall k,\ell,
...
\norm{\F_{k,\ell}^{(\UL)}}_F^2 \le P_k,
\qquad \forall k,\ell.
\label{eq:ul-fixed-power}
\end{align}
\end{subequations}

The key difference from payload completion is that the the block count $L_k$ $L$ is
...
\subsection{Downlink Design Problem}

Let $\Aset_\ell$ be the active-user set in block $\ell$. For each
$k\in\Aset_\ell$, let $b_{k,\ell}^{\mathrm{req}}$ denote the requested bits to be transmitted
of the current block, where
$b_{k,\ell}^{\mathrm{req}} = B_{k,\mathrm{rem}}^{(\ell)}$ in payload
completion and $b_{k,\ell}^{\mathrm{req}} = \bar b_{k,\ell}$ in fixed
continuous communication. The network objective remains the total latency
\(
\tau_{\mathrm{sum}}
=
\sum_{k\in\Kset}\frac{1}{f_{s,k}}\sum_{\ell'} n_{k,\ell'}
\).
During block $\ell$, only the active-user sub-blocklengths of the current block are
free, so the one-block downlink problem is written as
\begin{subequations}
\begin{align}
...
\eqref{eq:dl-disturbance-cov}. Thus, even if only per User's precoder is
changed, the feasible rates of all other active users are altered as well.
Hence the current-block decision cannot be separated from the coupled
active-user state of the block.
...
In fixed continuous communication mode, the the block count $L_k$ is prescribed and the
blockwise transmission bits targets are fixed. Again, $b_{k,\ell}$ denotes the transmitted
bits, whereas the design variables are the sub-blocklengths and precoders:
\begin{subequations}
\begin{align}
\min_{\{n_{k,\ell},\F_{k,\ell}^{(\DL)}\}}
\quad
&
\sum_{k\in\Kset}\frac{1}{f_{s,k}}\sum_{\ell=1}^{L} n_{k,\ell}
...
block is solved online through a constrained primal-dual update. In payload
completion, $b_{k,\ell}^{\mathrm{req}} = B_{k,\mathrm{rem}}^{(\ell)}$, whereas
in fixed continuous communication,
$b_{k,\ell}^{\mathrm{req}} = \bar b_{k,\ell}$.

For uplink block $(k,\ell)$ with requested service $b_{k,\ell}^{\mathrm{req}}$, define the rate
constraint residual
\begin{equation}
g_{k,\ell}^{(\UL,r)}
=
...
current request has already been met. In payload completion mode, the
...
\right\}.
\label{eq:ul-full-block-service-payload}
\end{equation}
...
\floor{T_k\,R_{k,\ell}^{(\UL)}(T_k)}
\right\}.
\label{eq:ul-full-block-service-fixed}
\end{equation}

sub-blocklength reduction is attempted only after the current block request has
been fully met. In payload completion, this means
$b_{k,\ell}^{(T)} = B_{k,\mathrm{rem}}^{(\ell)}$. In fixed continuous
communication, this means $b_{k,\ell}^{(T)} = \bar b_{k,\ell}$. If the current
request is only partially served, the block is committed at $n_{k,\ell}=T_k$
and the method moves to the next request without trying to reduce
sub-blocklength.
...
\begin{algorithmic}[1]
\Require Scenario mode, user $k$, payload $B_k$ or prescribed targets $\{\bar b_{k,\ell}\}$, tolerances $\epsilon_p,\epsilon_c,\epsilon_s$
\Ensure Block sequence $\{(b_{k,\ell},n_{k,\ell},\F_{k,\ell}^{(\UL)})\}$
\If{payload-completion mode}
    \State Set $B_{k,\mathrm{rem}}^{(1)} \gets B_k$
\EndIf
\State Initialize $\ell \gets 1$
\While{a new block request exists}
    \If{payload-completion mode}
        \State Set $b_{k,\ell}^{\mathrm{req}} \gets B_{k,\mathrm{rem}}^{(\ell)}$
    \Else
        \State Set $b_{k,\ell}^{\mathrm{req}} \gets \bar b_{k,\ell}$
    \EndIf
    \State Set $n_{k,\ell}\gets T_k$ and initialize $\F_{k,\ell}^{(\UL)}$, $\lambda_{k,\ell}^{(\UL,r)}$, and $\lambda_{k,\ell}^{(\UL,p)}$
    \Repeat
    ...
    \State Restore the best feasible state if one exists; otherwise restore the best minimum-violation state
    ...
    \ElsIf{$b_{k,\ell}^{(T)} < b_{k,\ell}^{\mathrm{req}}$}
        ...
        \State Do not reduce sub-blocklength in this block
    \Else
        \State Set $b_{k,\ell}=b_{k,\ell}^{\mathrm{req}}$
        \While{a smaller candidate sub-blocklength remains feasible after re-optimization}
            ...
            \State Restore the best feasible candidate state if one exists; otherwise restore the previously accepted state
            \If{$\dfrac{b_{k,\ell}}{n_{k,\ell}} \le R_{k,\ell}^{(\UL)}$ and $\norm{\F_{k,\ell}^{(\UL)}}_F^2 \le P_k$}
                \State Accept the candidate and continue
            \Else
                \State Restore the previous accepted sub-blocklength and stop reduction
            \EndIf
        \EndWhile
    ...
    \Else
        \State Record the shortfall $u_{k,\ell} \gets \bar b_{k,\ell} - b_{k,\ell}$
    \EndIf
...
sub-blocklength for the users whose full current requests are already
...
\begin{algorithmic}[1]
\Require Scenario mode, active-user set $\Aset_\ell$, remaining payloads $\{B_{k,\mathrm{rem}}^{(\ell)}\}_{k\in\Aset_\ell}$ or targets $\{\bar b_{k,\ell}\}_{k\in\Aset_\ell}$, tolerances $\epsilon_p,\epsilon_c,\epsilon_s$
...
\EndFor
\State Initialize the active-user precoders and multipliers
\Repeat
...
\EndFor
\State Set $b_{k,\ell}\gets 0$ for each user satisfying $b_{k,\ell}^{(T)}=0$
\State Remove users with zero supported service from the transmit set of the block
\If{the transmit set changes}
    \State Re-solve the reduced active-user block with the same primal-dual loop above
    \State Restore the best feasible reduced-set state if one exists; otherwise restore the previously accepted state
    \For{each remaining active user $k\in\Aset_\ell$}
        \State Recompute $b_{k,\ell}^{(T)}$ from \eqref{eq:dl-full-block-service}
    \EndFor
\EndIf
\For{each user satisfying $b_{k,\ell}^{(T)}=b_{k,\ell}^{\mathrm{req}}$}
    \While{a smaller candidate sub-blocklength preserves all already committed bits}
        \State Tentatively decrease $n_{k,\ell}$ while holding the other active-user sub-blocklengths fixed
        \State Re-optimize the affected active-user subset with the same primal-dual loop above
        \State Restore the best feasible candidate state if one exists; otherwise restore the previously accepted state
        \If{$\dfrac{b_{j,\ell}^{\mathrm{req}}}{n_{j,\ell}} \le R_{j,\ell}^{(\DL)}$ for all active users $j\in\Aset_\ell$ and $\sum_{j\in\Aset_\ell}\norm{\F_{j,\ell}^{(\DL)}}_F^2 \le P_B$}
            \State Accept the candidate and continue
        \Else
            \State Restore the previous accepted sub-blocklength of user $k$ and stop reducing that user
        \EndIf
    \EndWhile
\EndFor
...
payload completion, the number of bits to send is
$b_{k,\ell}^{\mathrm{req}} = B_{k,\mathrm{rem}}^{(\ell)}$. In fixed continuous
communication, it is
$b_{k,\ell}^{\mathrm{req}} = \bar b_{k,\ell}$.
...
the requested bit count encoded in query $q$, and $P(q,\theta_k)$ is the
transmit power induced by the predicted precoder. The coefficient
$\lambda_k^{(r)}$ penalizes rate shortfall relative to the requested service,
...
\For{each training episode}
    ...
    \For{each visited block $\ell$ of user $k$}
        \State Set $b_{k,\ell}^{\mathrm{req}} \gets B_{k,\mathrm{rem}}^{(\ell)}$ in payload completion, or $b_{k,\ell}^{\mathrm{req}} \gets \bar b_{k,\ell}$ in fixed continuous communication
        ...
        \EndWhile
        \If{payload-completion mode}
            \State Advance the episode remainder according to the committed service of the generating schedule
        \Else
            \State Advance to the next prescribed target of the generating schedule
        \EndIf
    \EndFor
\EndFor
...
\begin{algorithmic}[1]
\Require Scenario mode, trained uplink model $\theta_k$, user $k$, payload $B_k$ or prescribed targets $\{\bar b_{k,\ell}\}$
...
\For{each visited block $\ell$}
    ...
    \Else
        ...
        \While{a smaller candidate sub-blocklength exists}
            ...
            \If{$\dfrac{b_{k,\ell}^{\mathrm{req}}}{n_{k,\ell}} \le R_{k,\ell}^{(\UL)}(n_{k,\ell})$ and $P(q_{k,\ell}^{(\UL)},\theta_k)\le P_k$}
                \State Accept the reduction and continue
            \Else
                \State Restore the previous sub-blocklength and stop reduction
            \EndIf
        \EndWhile
    \EndIf
    \If{payload-completion mode}
...
completion, the number of bits to send is
$b_{k,\ell}^{\mathrm{req}} = B_{k,\mathrm{rem}}^{(\ell)}$. In fixed continuous
communication, it is
$b_{k,\ell}^{\mathrm{req}} = \bar b_{k,\ell}$.
...
\For{each training episode}
    \For{each visited communication block $\ell$}
        \State Determine the active-user set $\Aset_\ell$
        \State Store the queried-user set as $\Aset_\ell^{\mathrm{req}}\gets \Aset_\ell$
        \For{each $k\in\Aset_\ell$}
            \State Set $b_{k,\ell}^{\mathrm{req}} \gets B_{k,\mathrm{rem}}^{(\ell)}$ in payload completion, or $b_{k,\ell}^{\mathrm{req}} \gets \bar b_{k,\ell}$ in fixed continuous communication
        ...
        \While{the current joint sub-blocklength state is under consideration}
            ...
            \State Evaluate the current joint state and compute, for each $k\in\Aset_\ell$,
            \[
            b_{k,\ell}^{(\mathbf n)}
            =
            \min\!\left\{
            b_{k,\ell}^{\mathrm{req}},
            \floor{n_{k,\ell}\,R_{k,\ell}^{(\DL)}(\mathbf n_\ell)}
            \right\}
            \]
            \State Remove users with $b_{k,\ell}^{(\mathbf n)}=0$ from the transmit set if such users exist
            \If{no user satisfies $b_{k,\ell}^{(\mathbf n)} = b_{k,\ell}^{\mathrm{req}}$}
                \State Stop collecting additional queries for this block
            \Else
                \State Decrease the candidate sub-blocklengths of users already satisfying $b_{k,\ell}^{(\mathbf n)} = b_{k,\ell}^{\mathrm{req}}$
            \EndIf
        \EndWhile
        \If{payload-completion mode}
            \State Advance the episode remainders according to the committed service of the generating schedule
        \Else
            \State Advance to the next prescribed downlink block of the generating schedule
        \EndIf
    \EndFor
\EndFor
...
sub-blocklength reduction is attempted only for users whose full requested service
is already supported at the current state.

\begin{algorithm}[H]
\caption{Downlink Testing Stage}
\label{alg:dl-test-stage}
\begin{algorithmic}[1]
\Require Scenario mode, trained downlink model $\theta$, remaining payloads $\{B_{k,\mathrm{rem}}^{(\ell)}\}$ or prescribed targets $\{\bar b_{k,\ell}\}$
...
\For{each communication block $\ell$}
    ...
    \For{each $k\in\Aset_\ell$}
        ...
        \State Initialize $n_{k,\ell}\gets T_k$
    \EndFor
    \State Infer the joint downlink precoders at the full sub-blocklength vector
    \State Compute, for each $k\in\Aset_\ell$,
    \[
    b_{k,\ell}^{(T)}
    =
    \min\!\left\{
    b_{k,\ell}^{\mathrm{req}},
    \floor{T_k\,R_{k,\ell}^{(\DL)}(\mathbf n_\ell)}
    \right\}
    \]
    \State Set $b_{k,\ell}\gets 0$ for each user satisfying $b_{k,\ell}^{(T)}=0$
    \State Remove users with $b_{k,\ell}^{(T)}=0$ from the transmit set and re-infer the joint precoders if the active-user set changes
    \For{each user satisfying $b_{k,\ell}^{(T)} = b_{k,\ell}^{\mathrm{req}}$}
        \While{a smaller candidate sub-blocklength exists for that user}
            ...
            \If{$\dfrac{b_{j,\ell}^{\mathrm{req}}}{n_{j,\ell}} \le R_{j,\ell}^{(\DL)}(\mathbf n_\ell)$ for all active users $j$ and $\sum_{j\in\Aset_\ell}P_j(q_{\ell}^{(\DL)},\theta)\le P_B$}
                \State Accept the reduction
            \Else
                \State Restore the previous sub-blocklength of user $k$ and stop reducing that user
            \EndIf
        \EndWhile
    \EndFor
    \For{each remaining active user $k\in\Aset_\ell$}
...
development proceeds from a common finite-sub-blocklength model and communication
scenarios, to separate uplink and downlink formulations, and then to two
solution frameworks.
