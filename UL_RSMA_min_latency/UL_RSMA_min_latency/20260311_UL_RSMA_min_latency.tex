...
\begin{equation*}
	\begin{split}
		\min_{\F_{k,\ell}, n_{k,\ell}} \;& \sum_{k \in K} w_k \frac{n_{k,\ell}}{f_s} \\
...
\textbf{Asynchronicity Optimization}

Once subblocklength optimization process is over, we would obtain a list of all possible $n_{k,\ell}$ values for user-$k$ in the interval $[n_{min}, n_{max}]$.

To solve the asynchronicity  we can search through the possible $n_k$ values  for user-$k$  received from the previous alternating-optimization process to find the correct $n_k$ values for each user $k$ to minimize the following objectives,
...
&\quad 1\le n_{k,\ell}\le T_k,\;\; n_{k,\ell}, B_{k,\ell}\in\mathbb{Z}_{\ge 0},\;\; L_k\in\mathbb{Z}_{\ge 1},\\
...
\begin{equation*}
(n^\star_{k,\ell},\mathbf{F}^\star_{k,\ell})
=
...
\Big\{
B{_\text{rem}}^{(\ell)}_k,
B^\star_{k,\ell}(n^\star_{k,\ell},\F^\star_{k,\ell}, \epsilon_k)
\Big\}
\end{equation*}

If $B_{\text{rem}_k}^\ell$ is chosen, then implies all the bits have been transmitted.
...
where $B{_\text{rem}}^{(\ell)}_k$ is the remaining bits to transmit at each sub-block $\ell$ during the running of the algorithm,  $n^*_{k, \ell}$ is the optimal sub-blocklength of user $k$ sub-block $\ell$, $F^\star_{k, \ell}$ is the optimal precoder, $B^{\text{tx}}_{k, \ell}$ is the number of bits transmitted at user $k$ and sub-block $l$.


\subsection{Defining $B^\star_{k,\ell}(n_{k,\ell}, \F_{k, \ell}, \epsilon_k)$}

\begin{equation*}
B^\star_{k,\ell}(n_{k,\ell}, \F_{k, \ell})
=
\left\lfloor
n_{k,\ell} \times
\left(
...
\State $(n^\star_{k,\ell},\F^\star_{k,\ell}) \gets (n_{k,\ell}^{(i)},\F_{k, \ell}^{(i)})$
...
\lambda_{1,k,\ell}^{(i)}
+
\eta_{\lambda_1}^{(i)}\, g_{1,k,\ell}\!\left( \lambda_{1,k,\ell}^{(i)} \right)
\right]_+,
\qquad
\lambda_{2,k,\ell}^{(i+1)}
=
...
\lambda_{2,k,\ell}^{(i)}
+
\eta_{\lambda_2}^{(i)}\, g_{2,k,\ell}\!\left( \lambda_{2,k,\ell}^{(i)} \right)
\right]_+,
\end{equation*}
where $[x]_+ := \max(x,0.1 \times x)$ \textcolor{red}{because during the experiments, if we use $[x]_+ := \max(x, 0)$, then the rate after we optimized the precoder will be less}.
...
$T_k = \{10,15,20,30\}$, $\mathrm{SNR}=\{20,20,20,20\}$, $P_t=\{30,30,30,30\}$, $B_k = \{200,400,800,1600\}$.  \textcolor{red}{Even though $T_k$ should be determined by the Doppler shift of the agent and carrier frequency, but in this simulation, we chose to fix it.}  Here we simulate users with lower coherence blocklength $T$ having lower SNR and lower precoder power.
...
\begin{figure}[H]
    ...
    \end{subfigure}
    \caption{System-wide synchronization results for Experiment 0.  Note that white means 0.}
...
\begin{figure}[H]
    ...
    \end{subfigure}
    \caption{Overview of optimization performance for $K=100$.  \textcolor{red}{Cannot show CDF with histogram because CDF needs to sum up to 1.  We can omit the CDF curves here because showing the histogram is sufficient.}}
