...
\begin{equation*}
	n_k = L_k \times T_K
\end{equation*}
where $L_k$ is the number of coherent intervals within the blocklength of user $k$ and $T_k$ is the number of channel uses in one coherent interval of user $k$.
...
\begin{equation}
	\begin{split}
		\max_{\F_{k,\ell}}  \; C_k(\F_{k,l}) - V(\F_{k,\ell}) 
&=  \max_{\F_{k,\ell}} \; \log \det \Big( \sigma_w^{-2} \H_{k,\ell} \F_{k,\ell} \F_{k,\ell}^H \H_{k,\ell}^H \Big)
