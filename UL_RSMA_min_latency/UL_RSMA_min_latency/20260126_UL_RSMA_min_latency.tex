...
$L$ & Number of sub-blocks (coherence blocks) \\ \hline
...
$\mathbf{H} \in \mathbb{C}^{K \times L \times N_r \times N_t}$ & Channel matrix \\ \hline
$\mathbf{F} \in \mathbb{C}^{K \times L \times N_t \times d_k}$ & Precoder matrix \\ \hline
$\mathbf{X} \in \mathbb{C}^{K \times L \times d_k \times n_l}$ & Transmitted signal \\ \hline

$B_{k, \ell}$ & Number of bits transmitted for user $k$ in sub-block $\ell$ \\ \hline
$R^*_{k,\ell}(\H_\ell, \F_\ell, n_\ell)$ & Finite-blocklength rate for user $k$ in sub-block $\ell$ \\ \hline
$C_{k,\ell}(\H_\ell, \F_\ell, n_\ell)$ & Shannon capacity of channel $H_\ell$ \\ \hline
$V_{k,\ell}(\H_\ell, \F_\ell, n_\ell)$ & Channel dispersion of channel $H_\ell$ \\ \hline

$P_{k}$ & Transmit power constraint of user $k$\\ \hline
$M_\ell$ & Modulation order in sub-block $\ell$ \\ \hline
$\gamma_k$ & Signal-to-Interference-plus-Noise Ratio (SINR) \\ \hline
\end{tabular}
...
\begin{equation*}
	\begin{split}
		\min_{\F_{k,\ell}, n_{k}, \epsilon_k} \;& \sum_{k \in K} w_k \frac{n_k}{f_s} \\
		\text{s.t.}\;& \frac{B_{k,\ell}}{n_{k,\ell}} \leq \max_{\F_{k, \ell}} R^*_\ell(n_{k,\ell}, \F_{k, \ell},  \H_{k,\ell}), \quad \forall k \in \mcK \\ 
		& n_{k,l} \in [n_{\text{min}}, n_{\text{max}}], \quad \forall k \in \mcK \\
		& \|\F_{k,\ell}\|_F^2 \leq P_{k,\ell}, \quad \forall \ell \in [0, L_k],  \forall k \in \mcK.
	\end{split}
	%\label{eq_short_blocklength_latency_min_prob}
...
\begin{equation}
\begin{aligned}
\mathbf{A}_{k,\ell}(\H_{k,\ell},\mathbf{F}_{k,\ell})
&=
\sigma^{-2}
\mathbf{H}_{k,\ell}
\mathbf{F}_{k,\ell}
...
C_{k,\ell}(\H_{k,\ell}, \mathbf{F}_{k,\ell})
&=
\log_2 \det \big(
\mathbf{I}
+
\mathbf{A}_{k,\ell}(\H_{k,\ell},\mathbf{F}_{k,\ell})
\big), \\[2mm]
V_{k,\ell}(\H_{k,\ell}, \mathbf{F}_{k,\ell})
&=
(\log_2 e)^2
\sum_i
...
where $\lambda_i$ are the eigenvalues of $\mathbf{A}_{k,\ell}(\mathbf{F}_{k,\ell})$.
...
\Ensure
Feasible solutions
$\{(n_{k,\ell}^*, \mathbf{F}_{k,\ell}^*, R_{k,\ell}^*)\}$
...
\For{$k = 1$ \textbf{to} $K$} \Comment{Loop over users}
    ...
    \For{$\ell = 1$ \textbf{to} $L$} \Comment{Loop over sub-blocks}
...
\Ensure
Feasible tuples
$\{(n_\ell^*, \mathbf{F}_\ell^*, R_{\mathrm{FBL}}^*,
...
\Require
Precoder network $f_\theta$, initial precoder $\mathbf{F}$, number of epochs $E$, Lagrange multipliers $\lambda_{rate}, \lambda_{power}$,
learning rates $(\eta_{\text{net}}, \eta_{rate}, \eta_{power})$

\Ensure
Optimized precoder $\mathbf{F}^*$, updated multipliers
...
\For{$e = 1$ \textbf{to} $E$}
    ...
    \State $\mathbf{F}_{out} \gets f_\theta(\mathbf{H}_{kl})$
...
\State $\mathbf{F}^* \gets f_\theta(\mathbf{H}_{kl})$
\State Project $\mathbf{F}^*$ to valid precoder form 
...
Trained precoders $\{\mathbf{F}_{k,\ell}\}$, channel statistics 
...
\For{$k = 1$ \textbf{to} $K$}
    ...
    \For{$\ell = 1$ \textbf{to} $L_k$}
        ...
        \State Load trained precoder $\mathbf{F}_{k,\ell}$
...
\hline
Symbols per sub-block(coherant block) $T_{k}$ & 250 & 250 \\
...
The initial parameters assume the number of sub-blocks $L$, this is a problem as the number of sub-blocks is derived from the equation $L = N/T$, where $T$ (coherance time) is the only channel constant.

Thus it is not right to set the initial value of $L$ as it is a derived quantity.

As the current algorithm reduces the sub-blocklength $n_{l}$,  this could create a scenario post-optimization where due to reducing $n_l$ and having constant $T$, sub-block $l$ has less symbols and could also contain the symbols of the next sub-block.
...
Lets remove initial values of $L_k$ and $n_{k,\ell}$ and rewrite the Initial Parameters to only contain Total Bits $B_k$ and coherance time $T_k$.
...
\hline
Symbols per sub-block(coherant block) $T_k$ & 250 & 250 \\
\hline
SNR (dB) & 5.0 & 5,0 \\
Transmit power $P_t$ & 20.0 & 20.0 \\
\hline
...
We will try to solve the problem of changing $L_k$ to a derived quantity by assuming initial value of $L_k = 1$ and $n_{k,1} = T_k$.

This means initially all of user bits $B_k$ is encoded into $T_k$ symbols($n_{k,1} = T_k$) within a single sub-block(coherant block)($L_k = 1$).
Thus initially the simulation has a high bit/symbol ratio of $B_k/n_{k,1} = B_k/T_k$ which corresponds to a higher modulation order of $M_{k, \ell} = 2^{B_k/T_k}$.

However this might not satisfy the rate and power constraints, then we try to optimize $L_k$ and $B_{k,\ell}$ to reduce the sub-blocklength($n_{k,\ell}$) to satisfy the constraints.

--\> If the sub-blocklength $n_{k,\ell} = T_k$ does not satisfy the rate and power constraints, then we reduce $B_{k,\ell}$ in the range $[B_k, 1]$ until the constraints are satisfied.
...
THus instead of fixing the number of sub-blocks and blocklengths a priori, we derive them dynamically from the total remaining bits and coherence time. Starting from a $T_k$-symbol transmission, the sub-blocklength is progressively decreased if the rate and power constraints are satisfied. If reduction is not possible, the transmitted bits are split across multiple sub-blocks.


I beleive this is also optimal as having less sub-blocks with its high sub-blocklength (ie: having sub-locklength($n_{k,\ell}$) $=$ coherance interval ($T_k$)) result in lower latency in the current simulation as compared to more sub-blocks with lower sub-blocklengths.
I do not have a proof for this but i beleive an intuitive explanation is the fact that having a lower $n_{k,\ell}$ results in a larger \textbf{dispersion penalty} thus reducing the overall rate. This results in a higher total $n_{k}$ and thus higher latency when you sum the smaller $n_{k,\ell}$ across more sub-blocks. 
Thus it is better to have less sub-blocks with higher $n_{k,\ell}$ than more sub-blocks with lower $n_{k,\ell}$.
...
Sub-block parameters $(n_{\ell,\min}, \Delta n_\ell)$

\Ensure
Number of sub-blocks $\{L_k\}$, sub-blocklengths $\{n_{k,\ell}^*\}$, precoders $\{\mathbf{F}_{k,\ell}^*\}$, rates $\{R_{k,\ell}^*\}$
...
\State \Return
$(\{L_k\}, \{n_{k,\ell}^*\}, \{\mathbf{F}_{k,\ell}^*\}, \{R_{k,\ell}^*\})$
