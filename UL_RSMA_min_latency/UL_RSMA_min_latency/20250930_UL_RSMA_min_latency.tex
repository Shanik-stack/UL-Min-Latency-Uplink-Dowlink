...
Case 1: when $V(P)$ is independant on precoder $\F_k$,
 \begin{equation*}
\begin{aligned}
V(P) \;=\; V_{\mathrm{iid}}(P) 
...
Case 2:  when $V(P)$ is dependant on precoder $\F_k$,
\begin{equation*}
\begin{aligned}
V(P) \;=\; V_{\mathrm{iid}}(P) 
&\;\approx\; \; \sum_{i=1}^{n_{\min}}\!\left[
...
where $\H_k$ denotes the channel matrix of user-k,$ \F_k $the precoding amtrix and $\sigma_w^2 $the nosie variance. This formulation assumes that the blocklength, i.e., the number of channel uses for transmitting a codeword, tends to infinity, which enables arbitrarily small error probability through sufficiently long channel codes.

In reality most low latency application operate in the dinite blocklength regime, where the number of transmitted symbols $n$ is limited. In this case in a Guassian Channel, the received signal of blocklength $n$,
\begin{equation*}
\Y = \sum_{k=1}^K \H_K \F_k  \S_k + \W
\end{equation*}

where the recieved signal $\Y \in \mathbb{C}^{N_R \times n}$, the transmitted signal $\S_k \in \mathbb{C}^{d_k \times n}$, the Gussian channel matrix $\H_k \in \mathbb{C}^{N_R \times N_k}$, the precoder matrix $\F \in \mathbb{C}^{ N_k \times d_k}$, the noise matrix $\W \in \mathbb{C}^{N_R \times n}$  and $d_k \leq N_k$.
 
However this assumes a fixed Guassian channel, we consider system that experiences Rayleigh fading, where the channel coefficients vary over time. Let $T$ denote the coherance time ie: the number of channel uses over which the channel remains constant. 

Then the blocklength(total channel uses) of a codeword can be written as, 
\begin{equation*}
n = L \times T
\end{equation*}
where $T$ is the number of channel uses over which the channel remains constant and $L$ is the number of coherant channels within one blocklength.

By considering the coherant intervals the receive signal can be written as, 

\begin{equation*}
\Y = \sum_{k=1}^K \sum_{l = 1}^L \H_{k, l} \F_{k, l}  \S_{k, l} + \W \ \in \complex^{L \times N_r \times T}
\end{equation*}
...
Let $B_k$ be number of bits sent by user-$k$, let $n_k$ be the blocklength of user-$k$, let $\F_k$ be the precoder of user-$k$.

Let blocklength if user-$k$ be split in coherant intervals as, 
\begin{equation*}
n_k = L_k \times T_{k}
\end{equation*}
where $L_k$ is the number of coherant intervals within the blocklength of user0$k$ and $T_{k}$ is the number of channel uses in one coherant interval of user-$k$.

A weighted latency minimisation problem can be defined as, 

\begin{equation}
\begin{split}
\min_{\F_k, n_k, \epsilon_k} \;& \sum_{k \in K} W_k \frac{n_k}{f_s} \\
\text{s.t.}\;& \frac{B_k}{n_k} \leq \max_{\F_{k, l}} \frac{1}{L_k} \sum_{l=1}^{L_k} R^*(T_{k}, \F_{k, l}), \quad \forall k \in \mcK \\ 
& n_k \in [n_{\text{min}}, n_{\text{max}}], \quad \forall k \in \mcK \\
& \|\F_{k,l}\|_F^2 \leq P_{k,l}, \quad \forall l \in [0, L_k],  \forall k \in \mcK.
\end{split}
\label{eq_short_blocklength_latency_min_prob}
\end{equation}

Where the Finite Blocklength rate for coherant interval $l$ of user-$k$ is defined as, 
\begin{equation*}
R^*(T_{k} , \F_{k,l}) =  C_k(\F_{k,l}) - \sqrt{ \frac{V_k(\F_{k,l})}{ T_{k, l} } } Q^{-1}  (\epsilon_k), \quad \forall l \in [0,L_k] \forall k \in \mcK\\
\end{equation*}
The Channel Variance of user-$k$ in coherance interval $l$ is defined as, 
...
&\quad + \frac{(\ln e)^2}{2} \Bigg(
      ...
      + \frac{2 \Big( \frac{N_{k, \min}}{N_k} - 1 \Big)}{\alpha_{k,l}} 
        \sum_{i=1}^{N_{k, \min}} \mathbb{E} \Big[ \frac{1}{\lambda_{k, l, i}^2} \Big]
      + \mathcal{O}\Big( \frac{1}{\alpha_{k,l}^2} \Big)
    \Bigg) \\[0.6em]
&\quad ,\alpha_{k,l} = \frac{P_{k,l}}{N_k}.
\end{aligned}
\end{equation*}

where $P_{k,l}$ is the SNR. However since we are assuming an isotrophic Rayliegh fading channel it is also equal to the total transmit powet thus, $P_{k,l} = \|\F_{k,l}\|_F^2 $.
...
\textbf{Solving for} $n_k$,

Solution for $n_k$ can be found through exhaustive search between $[n_{min}, n_{max}]$.

\textbf{Solving for $\F_{k,l}$},

 We will optimise $R^*{(T_k, \F_{k,l})}$ to reduce the latency by increasing the upper bound. To convert  $R^*{(T_k, \F_{k,l})}$ into a convex function we remove the square root from $\sqrt{\frac{V(\F_{k,l})}{T_k}} Q^{-1}{(\epsilon_k)}$ and we shall use $\Q_{k,l} = \F_{k,l}\F_{k,l}^H$ which is a concave function in the optimisation procedure.

\begin{equation*}
\begin{aligned}
\F_{k,l} : \;\; & \max_{\F_{k,l}}  \; C_k(\F_{k,l}) - \V(\F_{k,l}) \\
&=  \max_{\F_{k,l}} \; \log \det \Big( \sigma_w^{-2} \H_{k,l} \F_{k,l} \F_{k,l}^H \H_{k,l}^H \Big)
   - \frac{\V(\F_{k,l})}{T_k} Q^{-1}(\epsilon_k) \\
&= - \min_{\Q_{k.l} \succeq 0} \; - \log \det \Big( \sigma_w^{-2} \H_{k,l} \Q \H_{k,l}^H \Big)
   - \frac{\V(\Q_{k,l})}{T_k} Q^{-1}(\epsilon_k)
\end{aligned}
\end{equation*}

The solution for both $n_k$ and $\Q_{k,l}$ is obtained through alternating between these 2 optimisation problems.

Once optimal $\Q_{k,l}$ is obtained, $\F_{k,l}$ is found by approximating $\F_{k,l}$ through randomisation to obtain $\F_{k,l}$ that satisfies the constraint $\Q_{k,l} = \F_{k,l} \F_{k,l}^H$
