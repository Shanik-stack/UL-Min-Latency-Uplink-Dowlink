...
$\CNR_{k,\ell}^{(\UL)}$, $\CNR_{k,\ell}^{(\DL)}$ & Uplink and downlink channel-to-noise ratios used by the inverse-CNR-weighted objective. \\
$\omega_{k,\ell}^{(\UL)}$, $\omega_{k,\ell}^{(\DL)}$ & Per-block objective weights. In the unweighted mode they are equal to one. \\
...
The payload-completion objective above is the network-level design target
throughout the report. In addition, the current uplink and downlink solvers
support an optional inverse-CNR-weighted \emph{rate objective} inside the
per-block precoder update. This weighted mode does not modify the payload
constraints or the latency definition; it changes only how strongly each user's
finite-sub-blocklength rate enters the blockwise optimization and the
query-level training loss.
...
The rate-maximizing primal term may
optionally be reweighted by a nonnegative objective coefficient. In the
unweighted mode,
$\omega_{k,\ell}^{(\UL)}=\omega_{k,\ell}^{(\DL)}=1$. In the inverse-CNR mode,
users with smaller channel-to-noise ratio receive larger objective weights.

For uplink block index $\ell$, let $\mathcal{U}_\ell\subseteq\Kset$ denote the
user set used to normalize the block-$\ell$ uplink priorities. Define
\begin{equation}
\CNR_{k,\ell}^{(\UL)}
=
\frac{\norm{\Hc_{k,\ell}^{(\UL)}}_F^2}{\sigma_k^2},
\qquad
\widetilde{\omega}_{k,\ell}^{(\UL)}
=
\frac{1}{\CNR_{k,\ell}^{(\UL)}},
\qquad
k\in\mathcal{U}_\ell,
\label{eq:ul-inverse-cnr-raw}
\end{equation}
and normalize these inverse-CNR scores by their sum:
\begin{equation}
\omega_{k,\ell}^{(\UL)}
=
\frac{\widetilde{\omega}_{k,\ell}^{(\UL)}}{
\sum_{j\in\mathcal{U}_\ell}\widetilde{\omega}_{j,\ell}^{(\UL)}
},
\qquad
k\in\mathcal{U}_\ell.
\label{eq:ul-inverse-cnr-weight}
\end{equation}

For downlink block $\ell$, define
\begin{equation}
\CNR_{k,\ell}^{(\DL)}
=
\frac{
...
and then define
\begin{equation}
\widetilde{\omega}_{k,\ell}^{(\DL)}
=
\frac{1}{\CNR_{k,\ell}^{(\DL)}},
\qquad
k\in\Aset_\ell,
\label{eq:dl-inverse-cnr-raw}
\end{equation}
and normalize these inverse-CNR scores by their sum:
\begin{equation}
\omega_{k,\ell}^{(\DL)}
=
\frac{\widetilde{\omega}_{k,\ell}^{(\DL)}}{
\sum_{j\in\Aset_\ell}\widetilde{\omega}_{j,\ell}^{(\DL)}
},
\qquad
k\in\Aset_\ell,
\label{eq:dl-inverse-cnr-weight}
\end{equation}
...
stored query contributes equally to the empirical objective. When the
inverse-CNR objective mode is used, the corresponding objective weights are
deterministic functions of the query state through
\eqref{eq:ul-inverse-cnr-weight} and \eqref{eq:dl-inverse-cnr-weight}; hence
they do not need to be listed as independent query coordinates.
...
$\omega^{(\UL)}(q)$ is the query-level objective weight, equal to $1$ in the
unweighted mode and equal to the normalized inverse-CNR weight associated with
that query in the inverse-CNR mode. The coefficient $\lambda_k^{(r)}$
penalizes rate shortfall relative to the bits-to-send value, whereas
$\lambda_k^{(p)}$ penalizes power-budget violation.
...
\sum_{q\in\Qset^{(\DL)}}
\Biggl(
&-\sum_{k\in\Aset(q)} \omega_k^{(\DL)}(q)\,R_k(q;\theta)\\
&+ \sum_{k\in\Aset(q)}
\lambda_k^{(r)}
...
predicted downlink precoder. The coefficient $\omega_k^{(\DL)}(q)$ is the
query-level objective weight of active user $k$, equal to $1$ in the unweighted
mode and equal to the normalized inverse-CNR weight in the
inverse-CNR mode. The penalty coefficients $\lambda_k^{(r)}$ and
